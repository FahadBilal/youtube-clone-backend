...
const getChannelStats = asyncHandler(async (req, res) => {
  ...
  const channelStats = {
    totalSubscriber: totalSubscriber[0]?.subscriberCount,
  ...
  if (!channelStats) {
    throw new ApiError(400, "Failed to fetched the channel stats");
  }
...
const getChannelVideos = asyncHandler( async(req, res)=>{
    ...
    if(!videos){
        throw new ApiError(400,"Failed to fetched all Videos")
    }
